...
fn main() {
    ...
    let mut waveform = String::from("bench");
    ...
    match env::var_os("OSRF_PROFILE") {
        Some(p) => {
            ...
            if let Some(v) = t.get("waveform").and_then(|v| v.as_str()) {
                waveform = v.to_string();
            }
    ...
    let waveform_v = match waveform.as_str() {
        "bench" => "Bench",
        "fcc247" => "Fcc247",
        other => panic!("waveform = {other:?}; expected bench or fcc247"),
    };
    if waveform == "fcc247" {
        // 15.247 is a 902–928 MHz rule: a US-shaped build must not offer
        // the TV band plans.
        for p in &band_plans {
            let stem = p.trim_end_matches('*');
            if stem.starts_with("band470") {
                panic!(
                    "waveform = \"fcc247\" is a 902-928 MHz shape; band plan {p:?} is outside it"
                );
            }
        }
    }
    let generated = format!(
        "// @generated by build.rs from OSRF_PROFILE\n\
         ...
         const WAVEFORM: osrf_profile_t114_ui::LinkWaveform = osrf_profile_t114_ui::LinkWaveform::{waveform_v};\n",
    );
