...
- **Profiles are wiring shells**, not application logic. Per-deployment configuration (chemistry / power policy / role / TX source), the embassy task wrappers that need concrete types, and the boot orchestrator (`run()`) belong in the profile. Everything else — the UI state-machine driver loop, battery monitoring, soft-off coordination, settings persistence, helper functions — belongs in an app crate (`apps/ui_runtime` for the UI deployment, `apps/midi_node` for the link-only deployment).  Use small trait abstractions over concrete board types (`Watchdog`, `BatterySampler`, function pointers for `enter_system_off` / `vbus_present`) so the app crate stays HAL-agnostic.
- **Production panic handlers belong in the board crate, feature-gated.**  `osrf-board-t114`'s `panic-stage` feature exposes a `#[panic_handler]` that stages the message into a cross-reset `.uninit` buffer and `SCB::sys_reset()`s — paired with `panic_record::recover_pending_panic()` which the profile calls during boot.  Profiles that want the production behaviour enable the feature and drop `panic-probe`; smoke / blink profiles leave the feature off and continue using `panic-probe`.  Rust enforces a single `#[panic_handler]` per binary, so the feature gate keeps the two patterns from colliding.
- **Display drivers live in `drivers/display/<panel>/`**, generic over embedded-hal SPI / OutputPin / DelayNs.  Board crates depend on the driver and re-export a concrete type alias (e.g. `board::Display`) plus the framebuffer (e.g. `board::framebuffer::Framebuffer`).  Per-panel geometry (width / height / MADCTL / RAM offsets) stays inside the driver crate as `const`s for now — when a second ST7789-controller panel actually shows up, generalise via a `PanelConfig` struct rather than forking the driver.
