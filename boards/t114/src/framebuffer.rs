...
//!
//! Implements [`DrawTarget`] so the [`crate::Renderer`] (or any
//! `embedded-graphics`-compatible code) can paint into it
//! synchronously.  Pixels accumulate in `pixels`; the affected
//! rectangle is tracked in `dirty`.  After a render completes,
//! call [`crate::display::St7789Display::flush`] (async) to
//! stream the dirty region to the panel — that path uses
//! `Spim::write().await` which yields during DMA so other tasks
//! (notably `osrf_link_runtime::run_rx`) can run.
//!
...
//! BSS is the cost of admission; nRF52840 has enough headroom for
//! it (~240 KB available after SoftDevice reservations).
//!
//! Single-instance: the t114_ui profile declares one and reuses it
//! across renders.  The dirty box gets cleared on flush so each
//! render's diff effectively says "what changed since last flush."
...
impl Framebuffer {
    ...
    /// from `(0,0)` to `(FB_W-1, FB_H-1)`.  Used by [`flush`] to
    /// stream out via SPI.
    pub fn pixels(&self) -> &[u16; FB_PIXELS] {
        &self.pixels
    }

    /// Current dirty bounding box (inclusive), or `None` if
    /// nothing has been drawn since the last [`clear_dirty`].
